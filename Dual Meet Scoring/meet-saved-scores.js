...
function renderMeetScoreStorage(){const meet=typeof genderMeet==='function'?genderMeet():dualMeet(),area=ensureMeetScoreStorage(),name=meet.meetName||'';area.innerHTML=`<label>MEET NAME<input id="meet-score-name" maxlength="80" value="${escapeHtml(name)}" placeholder="e.g. Lane Lines vs Riverview" /></label><span class="meet-score-file-message">Save this meet to a JSON file, or load a meet score from a JSON file.</span>`}
